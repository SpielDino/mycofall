...
func _on_interacted(body: Variant) -> void:
	...
	invisible_collider.queue_free()
			...
			#DamageNumbers.display_text(tr("INFO_SOLVED"), text_position.global_position, 5)
			#var tween = get_tree().create_tween().set_trans(Tween.TRANS_CUBIC);
			#tween.parallel().tween_property(rounded_rock_small, "rotation", Vector3(0, -130, 0), 3)
			#tween.parallel().tween_property(rounded_rock_middle, "rotation", Vector3(0, -130, 0), 4)
			#tween.parallel().tween_property(rounded_rock_big, "rotation", Vector3(0, -130, 0), 5)
	#else:
		#if text_position:
